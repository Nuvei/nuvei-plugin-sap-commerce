...
public class DefaultNuveiSettleServiceTest {
    ...
    @Spy
    @InjectMocks
    private DefaultNuveiSettleService testObj;
    ...
    public void requestSafechargeTransaction_ShouldNoThrowException_WhenPaymentTransactionEntryModelIsAuthorized() throws SafechargeException {
        ...
        when(nuveiPaymentTransactionServiceMock.createPaymentTransactionEntry(paymentTransactionModelMock,
                NuveiTransactionStatus.PENDING.getCode(), PaymentTransactionType.CAPTURE)).thenReturn(paymentTransactionEntryModelTwoMock);
        ...
        when(nuveiSafechargeWrapperMock.createSettleTransaction(ISO_CODE, new BigDecimal(AMOUNT).toString(), CLIENT_UNIQUE_ID, REQUEST_ID))
                .thenReturn(settleTransactionRequestMock);
    ...
    public void requestSafechargeTransaction_shouldExecuteRequest() throws SafechargeException {
        ...
        when(baseSiteModelMock.getNuveiMerchantConfiguration()).thenReturn(nuveiMerchantConfigurationModelMock);
        doReturn(nuveiSafechargeWrapperMock).when(testObj).getNuveiSafechargeWrapper(nuveiMerchantConfigurationModelMock);
        doNothing().when(testObj).storeSafechargeRequest(orderModelMock, settleTransactionRequestMock);
        when(nuveiSafechargeWrapperMock.executeTransaction(settleTransactionRequestMock)).thenReturn(settleTransactionResponseMock);
        when(settleTransactionResponseMock.getStatus()).thenReturn(Constants.APIResponseStatus.SUCCESS);
        doNothing().when(testObj).storeSafechargeResponse(orderModelMock, settleTransactionResponseMock);
        when(nuveiSafechargeWrapperMock.createSettleTransaction(ISO_CODE, new BigDecimal(AMOUNT).toString(), CLIENT_UNIQUE_ID, REQUEST_ID))
                .thenReturn(settleTransactionRequestMock);
        ...
        verify(nuveiSafechargeWrapperMock).createSettleTransaction(ISO_CODE, new BigDecimal(AMOUNT).toString(), CLIENT_UNIQUE_ID, REQUEST_ID);
    }
    ...
    public void requestSafechargeTransaction_shouldThrowException_WhenResponseStatusIsError() throws SafechargeException {
        ...
        doNothing().when(testObj).storeSafechargeRequest(orderModelMock, settleTransactionRequestMock);
        when(nuveiSafechargeWrapperMock.executeTransaction(settleTransactionRequestMock)).thenReturn(settleTransactionResponseMock);
        when(settleTransactionResponseMock.getStatus()).thenReturn(Constants.APIResponseStatus.ERROR);
        when(settleTransactionResponseMock.getErrorType()).thenReturn(Constants.ErrorType.INVALID_REQUEST);
        doNothing().when(testObj).storeSafechargeResponse(orderModelMock, settleTransactionResponseMock);
        when(nuveiSafechargeWrapperMock.createSettleTransaction(ISO_CODE, new BigDecimal(AMOUNT).toString(), CLIENT_UNIQUE_ID, REQUEST_ID))
                .thenReturn(settleTransactionRequestMock);

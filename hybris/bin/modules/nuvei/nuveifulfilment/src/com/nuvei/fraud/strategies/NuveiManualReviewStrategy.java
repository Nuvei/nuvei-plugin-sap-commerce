...
public class NuveiManualReviewStrategy extends AbstractNuveiFraudStrategy implements NuveiStrategy<Pair<AbstractOrderModel, FraudServiceResponse>, FraudServiceResponse> {

    private static final Logger log = LogManager.getLogger(NuveiManualReviewStrategy.class);

    public NuveiManualReviewStrategy() {
        super(log);
    }
    ...
    public boolean isApplicable(final Pair<AbstractOrderModel, FraudServiceResponse> source) {
        final PaymentTransactionEntryModel paymentEntry = findFraudTransaction(source.getLeft());
        return Objects.nonNull(paymentEntry);
    }

    protected PaymentTransactionEntryModel findFraudTransaction(final AbstractOrderModel abstractOrderModel) {
        return Stream.ofNullable(abstractOrderModel.getPaymentTransactions())
                .flatMap(Collection::stream)
                .map(PaymentTransactionModel::getEntries)
                .flatMap(Stream::ofNullable)
                .flatMap(List::stream)
                .filter(this::isManualReviewTransaction)
                .findAny()
                .orElse(null);
    }


    protected boolean isManualReviewTransaction(final PaymentTransactionEntryModel paymentTransactionEntryModel) {
        return PaymentTransactionType.AUTHORIZATION.equals(paymentTransactionEntryModel.getType())
                ...
                && SystemDecisionEnum.REVIEW.equals(paymentTransactionEntryModel.getSystemDecision());
    }
}
